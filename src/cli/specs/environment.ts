...
export const ENVIRONMENT_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      'The three kinds are reached differently. A paired Orca server is a connection, selected with --environment <environment-id> (a unique name also resolves; host list prints the id because names can collide). An SSH target is a machine the connected Orca host reaches, selected with --host ssh:<id>. Passing one where the other belongs is the most common way to get an empty or missing-host answer.',
      ...
      'SSH rows report the app-owned connection and lifecycle status. Paired-server rows use fresh read-only probes, marked connectionSource: probe / [probe], not the desktop persistent connection. A successful status probe sets connected: true; a probe the server answered but rejected also sets connected: true with probeError status_rejected, since the reply proves contact; failed or unattempted probes omit connected and show connection unknown with a safe probeError.',
