...
it('preserves fence across real full, patch and before-unload writes on local and SSH hosts', () => {
  const store = createStore()
  for (const host of ['local', HOST_ID]) {
    store.setWorkspaceSession(
      { ...rendererSession('a'), legacyWorkerResumeFencesByPaneKey: { 'tab:leaf': true } },
      host
    )
    store.setWorkspaceSession(rendererSession('b'), host)
    expect(store.getWorkspaceSession(host).legacyWorkerResumeFencesByPaneKey).toEqual({
      'tab:leaf': true
    })
    store.patchWorkspaceSession({ activeTabId: 'c' }, host)
    expect(store.getWorkspaceSession(host).legacyWorkerResumeFencesByPaneKey).toEqual({
      'tab:leaf': true
    })
    store.stageWorkspaceSessionBeforeUnload(rendererSession('d'), host)
    expect(store.getWorkspaceSession(host).legacyWorkerResumeFencesByPaneKey).toEqual({
      'tab:leaf': true
    })
  }
...
it.each([undefined, HOST_ID])(
  'publishes old-client flags after old-writer replacement on %s',
  (host) => {
    const store = createStore()
    const paneKey = 'tab:11111111-2222-4333-8444-555555555555'
    const record = {
      paneKey,
      tabId: 'tab',
      worktreeId: WT,
    ...
    store.setWorkspaceSession(
      { ...oldWriter, legacyWorkerResumeFencesByPaneKey: { [paneKey]: true } },
      host
    )
    store.stageWorkspaceSessionBeforeUnload(oldWriter, host)
    expect(
    ...
    expect(record).not.toHaveProperty('automaticResumeBlockedBy')
    store.setWorkspaceSession(
      { ...store.getWorkspaceSession(host), legacyWorkerResumeFencesByPaneKey: {} },
      host
    )
    expect(
      store.getWorkspaceSession(host).sleepingAgentSessionsByPaneKey?.[paneKey]
