...
function endTurn(op, { aborted = false, error = false } = {}) {
  ...
  send({ type: "turn_end", message: { stopReason }, ...(error ? { errorMessage: "scripted turn error" } : {}) });
  liveTurn = null;
...
function streamTextDeltas(text) {
  ...
  send({ type: "message_update", assistantMessageEvent: { kind: "text_delta", contentIndex: 0, delta: text.slice(0, mid) } });
  send({ type: "message_update", assistantMessageEvent: { kind: "text_delta", contentIndex: 0, delta: text.slice(mid) } });
  send({ type: "message_update", assistantMessageEvent: { kind: "text_end", contentIndex: 0, content: text } });
}

function streamThinking(thinking) {
  send({ type: "message_update", assistantMessageEvent: { kind: "thinking_start", contentIndex: 1 } });
  send({ type: "message_update", assistantMessageEvent: { kind: "thinking_delta", contentIndex: 1, delta: thinking } });
  send({ type: "message_update", assistantMessageEvent: { kind: "thinking_end", contentIndex: 1, content: thinking } });
}
...
function streamTool(callId, name, output, isError = false) {
  ...
  send({ type: "tool_execution_start", toolCallId: callId, toolName: name, args: { path: "scripted" } });
  send({ type: "tool_execution_update", toolCallId: callId, toolName: name, partialResult: output.slice(0, 4) });
...
function handleCommand(cmd) {
  ...
  switch (type) {
    ...
    case "get_tree": {
      ...
      const build = (parentId) => (byParent.get(parentId ?? "") ?? []).map((entry) => ({ entry, children: build(entry.id) }));
    ...
    case "get_available_models":
      ...
      respond(true, {
        models: [
          ...
          { id: "text-model", name: "Text Model", provider: "script-provider", reasoning: false, supportsImages: false },
    ...
    case "get_available_thinking_levels":
      respond(true, { levels: ["low", "medium", "high"] });
      return;
    ...
    case "prompt": {
      ...
      if (text.includes("PROMPT-ME")) {
        ...
        send({ type: "extension_ui_request", id: dialogId, method: "confirm", title: "Scripted?", message: "Proceed?" });
        return;
      }
      respond(true, {});
      runTurn(text);
...
function runTurn(text) {
  ...
  if (text.includes("SLOW")) {
    ...
    send({ type: "turn_start" });
    send({ type: "message_update", assistantMessageEvent: { kind: "text_start", contentIndex: 0 } });
    send({ type: "message_update", assistantMessageEvent: { kind: "text_delta", contentIndex: 0, delta: reply.slice(0, 8) } });
    setTimeout(() => {
      if (!liveTurn) {
        return;
      }
      send({ type: "message_update", assistantMessageEvent: { kind: "text_delta", contentIndex: 0, delta: reply.slice(8) } });
      send({ type: "message_update", assistantMessageEvent: { kind: "text_end", contentIndex: 0, content: reply } });
...
function handleLine(line) {
  ...
  try {
    ...
    send({ type: "response", command: cmd?.type ?? "unknown", success: false, error: error.message });
  }
}
