// Transplanted from 44madfire/orca-pi `packages/structured-bridge/src/pi-translator.ts`
// (MIT) for SNC1.9 native Pi. Only the event/row types are re-pointed at
// `pi-session-events`; coalescing, dedupe, lifecycle, and journal decisions
// are verbatim.
/**
...
export type TranslatorToolState = {
...
export type TranslatorJournalEntry = {
...
export class PiTranslator {
  ...
  // Protected (not private): write-only lifecycle markers the compiler's
  // unused-field gate would otherwise reject on a private they never read.
  protected sawTurnStart = false;
  protected sawSettled = false;
  ...
  applyPiRecord(record: Record<string, unknown>): PiTranslatorEvent[] {
    ...
    for (const event of events) {
      switch (event.type) {
        ...
        // No default: the event union is closed, so every case above is
        // exhaustive and an unknown kind cannot arrive typed. Untyped records
        // map to `[]` in `mapPiRecordToSessionEvents` before reaching here.
      }
    }
    return out;
  }
