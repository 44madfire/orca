// Transplanted from 44madfire/orca-pi `packages/structured-bridge/src/pi-mapping.ts`
// (MIT) for SNC1.9 native Pi. Bridge protocol types become Orca-native
// `pi-session-events` shapes (identical cases/fields); pure mapping bodies
// are verbatim. Function names are Pi-native (`Bridge` → `Pi`).
/**
...
import type { PiPromptImage, PiSessionEvent, PiSessionOptions } from "./pi-session-events";

export type PiDispatchMessage = {
  text: string;
  images?: PiPromptImage[];
};

type PiProviderCapabilities = {
...
export function piPiProviderCapabilities(model?: string): PiProviderCapabilities {
...
export type PiDispatchValidation = {
  ok: boolean;
...
export function validatePiDispatch(
  ...
  if (images.length > 0) {
    if (model !== undefined && !piPiProviderCapabilities(model).images) {
 ...
 */
export function mapPiDispatchToPromptCommand(
  dispatchOpId: string,
  message: PiDispatchMessage,
 ...
 */
export function mapPiRecordToSessionEvents(record: Record<string, unknown>): PiSessionEvent[] {
