// Transplanted from 44madfire/orca-pi `packages/structured-bridge/src/pi-history.ts`
// (MIT) for SNC1.9 native Pi. Only the history-row type is re-pointed at
// `pi-session-events`; branch extraction and translation are verbatim.
/**
...
import type { PiHistoryRow } from "./pi-session-events";

export type PiHistoryEntryLike = {
...
export type PiHistoryTreeNodeLike = {
 ...
 */
export function translatePiEntryToHistoryRows(entry: PiHistoryEntryLike): PiHistoryRow[] {
  try {
 ...
 */
export function translatePiBranchToHistory(branch: readonly PiHistoryEntryLike[]): PiHistoryRow[] {
  const out: PiHistoryRow[] = [];
  for (const entry of branch) {
    const rows = translatePiEntryToHistoryRows(entry);
