import { SessionHostPartitionOperations } from '../persistence/loading-store/session-host-partitions'
...
describe('settled worker automatic-resume fence persistence', () => {
  ...
  function harness(
    ...
    const partitions = new SessionHostPartitionOperations(
      {
        state: {
    ...
    const store = {
      getWorkspaceSession: () => partitions.getWorkspaceSession(),
    ...
    return {
      ...
      recordFlag: () =>
        partitions.getWorkspaceSession().sleepingAgentSessionsByPaneKey?.[PANE_KEY]
  ...
  // Main derives compatibility on reads, including before any renderer writes.
  it('publishes the legacy flag for an older client without a newer renderer', () => {
    const h = harness()
    settle(h.db, h.taskId, h.dispatchId)

    h.persistence.prepare()

    expect(h.recordFlag()).toBe('legacy-orchestration-worker')
  })
