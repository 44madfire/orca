...
export async function readCodexStructuredSessionOptions(input: {
  ...
  // `models` stays what `model/list` offered: a current id the account cannot select
  // is still reported (the thread runs it) but never listed, so no raw id is offerable.
