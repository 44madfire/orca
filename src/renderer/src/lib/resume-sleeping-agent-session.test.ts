...
describe('resumeSleepingAgentSessionsForWorktree', () => {
  ...
  it('rechecks pane ownership after an earlier fresh resume activates a new terminal', () => {
    ...
    expect(state.sleepingAgentSessionsByPaneKey[initiallyUnowned.paneKey]).toEqual(initiallyUnowned)
    expect(state.sleepingAgentSessionsByPaneKey[initiallyOwned.paneKey]).toEqual(initiallyOwned)
  })
  ...
  it('does not let mixed legacy provider sessions claim the whole preserved tab', () => {
    ...
    expect(state.sleepingAgentSessionsByPaneKey[first.paneKey]).toEqual(first)
    expect(state.sleepingAgentSessionsByPaneKey[second.paneKey]).toEqual(second)
  })
  ...
  it('launches once and clears skipped duplicates for the same provider session', () => {
    ...
    expect(state.sleepingAgentSessionsByPaneKey[duplicate.paneKey]).toEqual(duplicate)
  })
  ...
  it('does not let completed same-provider hibernation evidence launch before an active fresh resume', () => {
    ...
    expect(state.sleepingAgentSessionsByPaneKey[active.paneKey]).toEqual(active)
  })
  ...
  it('does not let invalid pane-owned records block a valid duplicate resume', () => {
    ...
    expect(state.sleepingAgentSessionsByPaneKey[valid.paneKey]).toEqual(valid)
  })
