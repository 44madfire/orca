...
Pass arbitrary specs as one argv element with shell execution disabled. Never
interpolate spec text into a double-quoted shell command: dollar-paren and backtick
substitutions execute before Orca starts and can hang. In POSIX shells, single-quote
each value, replacing every embedded apostrophe with `'"'"'`; newlines remain literal
inside the quotes. For example: `--spec 'Review the worker'"'"'s $(literal) text'`.
JSON escaping is not shell escaping. On Windows, use a native argv launcher or the
selected shell's argument encoder; cmd.exe does not support POSIX single quoting.
